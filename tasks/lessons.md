...
- 2026-09-24 — Issue #3's brief redirected where the graph is read but not who writes it or which other commands read it; three extra rounds found `graph-qml.py` writing a branch's QML into master's graph, raw `graphify` commands failing, and a worktree-local build shadowing master's → when a brief moves a shared path, list every reader and writer of it (scripts, CLI commands, docs) and decide each one in the brief.
- 2026-09-24 — `git add … && git commit …` in one Bash call was blocked twice: the `check-docs.py` PreToolUse hook runs before the command, so it sees only what is already staged → run `git add` and `git commit` as separate Bash calls.
