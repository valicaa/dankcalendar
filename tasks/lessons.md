...
- 2026-09-24 — Guarded shell blocks only against unset variables; an empty or wrong substituted path still let `rm -rf` run in $HOME → any destructive block first asserts it is in the expected repo/branch (`git rev-parse --show-toplevel`, `git branch --show-current`) with `|| exit 1`, and uses git commands over bare `rm -rf`. TEST
